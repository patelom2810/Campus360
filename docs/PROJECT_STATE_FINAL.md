...
- **API Model Upgrade:**
  - Testing against Google Gemini API identified that `gemini-3.5-flash-lite` provides sub-second latency (~0.8s) and high Free-Tier quota availability (avoiding the 20 request/day limit of `gemini-3.6-flash`).
  - Configured `src/genai/insights.py` to prioritize `gemini-3.5-flash-lite` with modern flash fallbacks (`gemini-3.5-flash`).
...
- GenAI API endpoints were called live using `gemini-3.5-flash-lite`, verified against hallucinated figures, and stress-tested with invalid keys.
